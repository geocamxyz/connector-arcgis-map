var Ue = typeof window < "u";
const we = {
  Promise: Ue ? window.Promise : void 0
};
var xe = "4.25", te = "next";
function Ae(e) {
  if (e.toLowerCase() === te)
    return te;
  var r = e && e.match(/^(\d)\.(\d+)/);
  return r && {
    major: parseInt(r[1], 10),
    minor: parseInt(r[2], 10)
  };
}
function Ne(e) {
  return e === void 0 && (e = xe), "https://js.arcgis.com/".concat(e, "/");
}
function Pe(e) {
  e === void 0 && (e = xe);
  var r = Ne(e), t = Ae(e);
  if (t !== te && t.major === 3) {
    var o = t.minor <= 10 ? "js/" : "";
    return "".concat(r).concat(o, "esri/css/esri.css");
  } else
    return "".concat(r, "esri/themes/light/main.css");
}
function Ze(e) {
  var r = document.createElement("link");
  return r.rel = "stylesheet", r.href = e, r;
}
function He(e, r) {
  if (r) {
    var t = document.querySelector(r);
    t.parentNode.insertBefore(e, t);
  } else
    document.head.appendChild(e);
}
function Re(e) {
  return document.querySelector('link[href*="'.concat(e, '"]'));
}
function Ye(e) {
  return !e || Ae(e) ? Pe(e) : e;
}
function We(e, r) {
  var t = Ye(e), o = Re(t);
  return o || (o = Ze(t), He(o, r)), o;
}
var qe = {};
function Be(e) {
  var r = document.createElement("script");
  return r.type = "text/javascript", r.src = e, r.setAttribute("data-esri-loader", "loading"), r;
}
function pe(e, r, t) {
  var o;
  t && (o = Ce(e, t));
  var a = function() {
    r(e), e.removeEventListener("load", a, !1), o && e.removeEventListener("error", o, !1);
  };
  e.addEventListener("load", a, !1);
}
function Ce(e, r) {
  var t = function(o) {
    r(o.error || new Error("There was an error attempting to load ".concat(e.src))), e.removeEventListener("error", t, !1);
  };
  return e.addEventListener("error", t, !1), t;
}
function ze() {
  return document.querySelector("script[data-esri-loader]");
}
function ne() {
  var e = window.require;
  return e && e.on;
}
function Ve(e) {
  e === void 0 && (e = {});
  var r = {};
  [qe, e].forEach(function(a) {
    for (var d in a)
      Object.prototype.hasOwnProperty.call(a, d) && (r[d] = a[d]);
  });
  var t = r.version, o = r.url || Ne(t);
  return new we.Promise(function(a, d) {
    var b = ze();
    if (b) {
      var x = b.getAttribute("src");
      x !== o ? d(new Error("The ArcGIS API for JavaScript is already loaded (".concat(x, ")."))) : ne() ? a(b) : pe(b, a, d);
    } else if (ne())
      d(new Error("The ArcGIS API for JavaScript is already loaded."));
    else {
      var v = r.css;
      if (v) {
        var U = v === !0;
        We(U ? t : v, r.insertCssBefore);
      }
      b = Be(o), pe(b, function() {
        b.setAttribute("data-esri-loader", "loaded"), a(b);
      }, d), document.body.appendChild(b);
    }
  });
}
function be(e) {
  return new we.Promise(function(r, t) {
    var o = window.require.on("error", t);
    window.require(e, function() {
      for (var a = [], d = 0; d < arguments.length; d++)
        a[d] = arguments[d];
      o.remove(), r(a);
    });
  });
}
function Xe(e, r) {
  if (r === void 0 && (r = {}), ne())
    return be(e);
  var t = ze(), o = t && t.getAttribute("src");
  return !r.url && o && (r.url = o), Ve(r).then(function() {
    return be(e);
  });
}
const Ie = (e, r) => e.replace(/\(\?\<(.+?)\>[^)]*\)/g, (t, o) => r[o]);
Number.prototype.toRad = function() {
  return this * (Math.PI / 180);
};
Number.prototype.toDeg = function() {
  return this * (180 / Math.PI);
};
Number.prototype.to2DP = function() {
  return parseFloat(this.toFixed(2));
};
const he = function(e) {
  return Array.isArray(e) ? e : [
    e.longitude || (e.coords ? e.coords.longitude : e.geometry ? e.geometry.longitude : null),
    e.latitude || (e.coords ? e.coords.latitude : e.geometry ? e.geometry.latitude : null)
  ];
}, ve = function(e, r) {
  const [t, o] = he(e), [a, d] = he(r), b = a.toRad(), x = d.toRad(), v = t.toRad(), U = o.toRad(), P = Math.sin(b - v) * Math.cos(x), Z = Math.cos(U) * Math.sin(x) - Math.sin(U) * Math.cos(x) * Math.cos(b - v);
  return (Math.atan2(P, Z).toDeg() + 360) % 360;
}, V = (e, r = {}, t = "") => {
  const o = document.createElement(e);
  for (let a in r)
    o.setAttribute(a, r[a]);
  return o.innerHTML = t, o;
}, De = (e, r) => (document.getElementById(e) || document.getElementsByTagName("head")[0].prepend(V("STYLE", { type: "text/css" }, r)), !0), Je = 600, Fe = 8, ye = (e, r) => {
  const t = Number(e);
  return Number.isFinite(t) && t > 0 ? t : r;
}, F = (e, r) => {
  if (Array.isArray(e))
    return [0, 1, 2].map((t) => ye(e[t], r));
  if (typeof e == "string")
    try {
      const t = JSON.parse(e);
      return Array.isArray(t) ? F(t, r) : F([t, t, t], r);
    } catch {
      return [r, r, r];
    }
  if (typeof e == "number") {
    const t = ye(e, r);
    return [t, t, t];
  }
  return [r, r, r];
}, Qe = "data:image/png;base64,", Ke = function(e = {}) {
  De("geocam-argis-map", `
      .geocam-auto-rotate-checkbox {
        display: none;
      }

      .geocam-auto-rotate-span {
        background-image: url("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' enable-background='new 0 0 24 24'  viewBox='0 0 24 24' fill='currentColor'><rect fill='none' height='24' width='24'/><path d='M7.94,5.12L6.49,3.66C8.07,2.61,9.96,2,12,2c5.52,0,10,4.48,10,10c0,2.04-0.61,3.93-1.66,5.51l-1.46-1.46 C19.59,14.86,20,13.48,20,12c0-4.41-3.59-8-8-8C10.52,4,9.14,4.41,7.94,5.12z M17.66,9.53l-1.41-1.41l-2.65,2.65l1.41,1.41 L17.66,9.53z M19.78,22.61l-2.27-2.27C15.93,21.39,14.04,22,12,22C6.48,22,2,17.52,2,12c0-2.04,0.61-3.93,1.66-5.51L1.39,4.22 l1.41-1.41l18.38,18.38L19.78,22.61z M16.06,18.88l-3.88-3.88l-1.59,1.59l-4.24-4.24l1.41-1.41l2.83,2.83l0.18-0.18L5.12,7.94 C4.41,9.14,4,10.52,4,12c0,4.41,3.59,8,8,8C13.48,20,14.86,19.59,16.06,18.88z'/></svg>")
      }

      .geocam-auto-rotate-checkbox:checked +.geocam-auto-rotate-span {
          background-image: url("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' enable-background='new 0 0 24 24' viewBox='0 0 24 24' fill='currentColor'><rect fill='none' height='24' width='24'/><path d='M18.6,19.5H21v2h-6v-6h2v2.73c1.83-1.47,3-3.71,3-6.23c0-4.07-3.06-7.44-7-7.93V2.05c5.05,0.5,9,4.76,9,9.95 C22,14.99,20.68,17.67,18.6,19.5z M4,12c0-2.52,1.17-4.77,3-6.23V8.5h2v-6H3v2h2.4C3.32,6.33,2,9.01,2,12c0,5.19,3.95,9.45,9,9.95 v-2.02C7.06,19.44,4,16.07,4,12z M16.24,8.11l-5.66,5.66l-2.83-2.83l-1.41,1.41l4.24,4.24l7.07-7.07L16.24,8.11z'/></svg>")
      }

    `);
  let t, o, a = [], d, b, x, v, U, P, Z, X, re, oe, I, Te, se, Le, ae = !0, ie;
  const { mapView: s, prevNextPlugin: Q, widgets: _e, expands: et, src: D } = e, H = document.createElement("div"), ce = function(n, i, c) {
    return {
      geometry: {
        type: "point",
        latitude: c,
        longitude: i
      },
      symbol: {
        type: "picture-marker",
        // autocasts as new PictureMarkerSymbol()
        url: Qe,
        width: "56",
        height: "101",
        angle: n
      }
    };
  };
  let k = ce(0, 0, 0);
  const ke = function(n) {
    return {
      attributes: {
        id: "lock"
      },
      geometry: {
        type: "point",
        latitude: n.latitude,
        longitude: n.longitude
      },
      symbol: {
        type: "simple-marker",
        // autocasts as new SimpleMarkerSymbol()
        path: "M12 4C7.31 4 3.07 5.9 0 8.98L12 21l5-5.01V8h5.92C19.97 5.51 16.16 4 12 4zm7 14h2v2h-2z M19 10h2v6h-2z",
        color: [255, 0, 0, 0.8],
        //"rgba(0,0,0,0)", for some reason if color isn't changed then outline colour doesn't change either.
        size: 24,
        // pixels
        xoffset: 0,
        yoffset: 12,
        outline: {
          // autocasts as new SimpleLineSymbol()
          color: [255, 255, 255, 1],
          width: 0
          // points
        }
      }
    };
  };
  let K = null;
  const R = function(n, i, c) {
    d && (d.removeAll(), t.visible() ? (H.classList.remove("esri-disabled"), n !== null && (Z() ? (s.rotation = n * -1, n = 0, (c || c === 0) && s.goTo({
      center: [i, c]
    })) : s.rotation = 0, k = ce(
      n || 0,
      i || k.geometry.longitude,
      c || k.geometry.latitude
    ), d.add(k), (c || c === 0) && ie([i, c]))) : H.classList.add("esri-disabled"));
  }, le = function(n, i, c = {}) {
    const l = new RegExp(i, "i");
    let p = l.test(n.name) || l.test(n.alias);
    return p && c.description && (p = !!n.description), p;
  }, ue = function(n) {
    if (n) {
      var i = document.createElement("textarea");
      return i.innerHTML = n, i.value;
    } else
      return "";
  }, Ee = (n, i) => {
    const c = n.base;
    if (n.filenames)
      return JSON.parse(i[n.filenames]).map((l) => Array.isArray(l) ? l.map((p) => /^https?:\/\//i.test(p) ? l : `${c}${p}`) : /^https?:\/\//i.test(l) ? l : `${c}${l}`);
    {
      const l = i[n.capture].split(".")[0], p = l.split("/").pop(), E = JSON.parse(i[n.lengths]), y = JSON.parse(i[n.offsets]);
      return E.map((A, m) => {
        const j = encodeURIComponent(
          `https://s3.us-west-004.backblazeb2.com/gc-raw-surveys-archive/${l}_${m}.tar`
        );
        return `${c}${p}/${m}/${i[n.shot]}.jpg?offset=${y[m]}&length=${A}&container=${j}`;
      });
    }
  }, je = (n) => {
    if (Array.isArray(n))
      return n.filter((i) => typeof i == "string");
    if (typeof n == "string")
      try {
        const i = JSON.parse(n);
        if (Array.isArray(i))
          return i.filter((c) => typeof c == "string");
      } catch {
        return [];
      }
    return [];
  }, Me = function(n) {
    return a.findIndex((i) => i.layer == n.layer);
  }, Ge = function(n) {
    if (t.label) {
      const { capture: i, utc_time: c, shot: l } = n, p = new Date(c);
      t.label(`${p.toLocaleString()}`);
    }
  };
  let de;
  const $ = function(n, i) {
    const c = I();
    console.log("shotclick with viewlock", c);
    const l = a[i], p = n.attributes[l.shot];
    de = p, t.shot(p), Q && (Q.prev(n.attributes.prev), Q.next(n.attributes.next));
    const E = [0, 1, 2].map(
      (M) => Ie(l.calibrationBase, {
        camera: M,
        rig_id: n.attributes[l.rigId],
        calibration: n.attributes[l.calibration]
      })
    ), y = n.attributes[l.yaw], Y = n.attributes[l.rotation], A = X() && l.brightness ? n.attributes[l.brightness] : null;
    re = A;
    const m = F(
      n.attributes.exposure_us,
      Je
    ), j = F(
      n.attributes.gain_boost,
      Fe
    ), z = {
      id: p,
      latitude: n.geometry && typeof n.geometry.latitude == "number" ? n.geometry.latitude : null,
      longitude: n.geometry && typeof n.geometry.longitude == "number" ? n.geometry.longitude : null,
      utc_time: n.attributes.utc_time || null,
      capture: n.attributes[l.capture] || null,
      exposure_us: m,
      gain_boost: j,
      tags: je(n.attributes.image_tags)
    }, w = Ee(l, n.attributes);
    if (c) {
      const M = ve(n.geometry, c);
      t.facing(M);
    }
    t.show(w, y, E, Y, A, z), R(
      t.facing(),
      n.geometry.longitude,
      n.geometry.latitude
    ), Ge(n.attributes);
  };
  let me, q, _;
  const ee = function(n, i, c, l) {
    clearTimeout(me), me = setTimeout(() => {
      const p = s.scale, E = Math.ceil(p / 500), y = s.extent, Y = `${y.xmin},${y.ymin},${y.xmax},${y.ymax},${y.spatialReference.wkid}`, A = `mod(id,${E}) = 0 AND extent = ${Y}`;
      a.forEach((m) => {
        m.layer.definitionExpression !== A && (m.layer.definitionExpression = A, console.log(
          "definition expression changed for",
          m.layer,
          A
        ));
      }), oe(s.zoom), R(t.facing());
    }, 500);
  }, Oe = function(n, i, c, l) {
    se([s.center.longitude, s.center.latitude]);
  };
  this.init = async function(n) {
    t = n, ie = t.store("marker"), oe = t.store("zoom"), se = t.store("center"), I = t.store("viewlock"), Z = t.store("autorotate"), P = V("DIV", { class: "geocam-auto-rotate" });
    const i = V("LABEL", { class: "geocam-auto-rotate-label" }), c = V("INPUT", {
      type: "checkbox",
      class: "geocam-auto-rotate-checkbox"
    }), l = V(
      "SPAN",
      { class: "geocam-auto-rotate-span geocam-viewer-control-button" },
      " Autorotate"
    );
    c.checked = Z(), c.addEventListener("change", () => {
      Z(c.checked);
    }), i.appendChild(c), i.appendChild(l), P.appendChild(i), t.addControl(P, "left-top"), x = Z((m) => {
      P.setAttribute(
        "title",
        m ? "turn auto-rotate off" : "turn auto-rotate on"
      ), R(
        t.facing(),
        k.geometry.longitude,
        k.geometry.latitude
      );
    }), X = t.store("autobrightness"), v = X((m) => {
      t.reload(m ? re : "[1,1,1]");
    }), U = t.visible((m) => R(t.facing()));
    const [p, E, y, Y, A] = await Xe([
      "esri/layers/GraphicsLayer",
      "esri/core/watchUtils",
      "esri/layers/FeatureLayer",
      "esri/widgets/Expand",
      "esri/renderers/support/jsonUtils"
    ]);
    if (s.when(async () => {
      s.on("clickable", (u) => {
        ae = u;
      }), s.on("key-down", (u) => {
        if (u && u.target && u.target.closest("input,calcite-input"))
          return;
        const f = [
          "ArrowUp",
          "ArrowDown",
          "ArrowRight",
          "ArrowLeft",
          "a",
          "d",
          "w",
          "s"
          // w and s don't seem to be used for map actions but just in case that changes in the future.
        ], g = u && u.key;
        t.visible() && f.indexOf(g) !== -1 && u.stopPropagation();
      }), s.on("immediate-click", (u) => {
        if (!ae) return;
        const f = {
          x: u.x,
          y: u.y
        };
        if (console.log("immediate-click", u, f), o) {
          console.log("space wqas down");
          const g = s.toMap(f);
          if (I(g), K && s.graphics.removeAll(), K = ke(g), s.graphics.add(K), t.visible()) {
            const T = ve(k.geometry, g);
            t.facing(T);
          }
        } else
          s.hitTest(f).then((g) => {
            if (g.results && g.results.length > 0)
              for (var T = 0; T < g.results.length; T++) {
                const L = g.results[T].graphic, N = Me(L);
                if (N >= 0) {
                  Object.entries(L.attributes).length < 2 ? L.layer.queryFeatures({
                    objectIds: [L.attributes.id],
                    returnGeometry: !0,
                    outFields: "*",
                    where: L.layer.definitionExpression
                  }).then((G) => {
                    G.features.length > 0 && $(G.features[0], N);
                  }) : $(L, N);
                  break;
                }
              }
          });
      });
      const m = document.createElement("div");
      m.className = "esri-widget--button", m.title = "Copy short URL to clipboad", m.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" height="24" width="24"><path d="M23 15H11.707l2.646 2.646-.707.707L9.793 14.5l3.854-3.854.707.707L11.707 14H23zm-13-5H6v1h4zm-4 5h2v-1H6zM3 4h3V3h3a2 2 0 0 1 4 0h3v1h3v9h-1V5h-2v2H6V5H4v16h14v-5h1v6H3zm4 2h8V4h-3V2.615A.615.615 0 0 0 11.386 2h-.771a.615.615 0 0 0-.615.615V4H7zM6 19h4v-1H6z"></path></svg>
    <span class="esri-icon-font-fallback-text">Copy short URL to clipboad</span>`, m.addEventListener("click", async () => {
        const u = `${document.location.origin}/🔗`;
        try {
          const g = await (await fetch(u, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              link: { url: document.location.toString() }
            })
          })).json();
          await navigator.clipboard.writeText(g.link), alert(`Url copied to clipboard: ${g.link}`);
        } catch (f) {
          alert(
            `Sorry, the short URL could not be copied to the clipboard: ${f}`
          );
        }
      }), s.ui.add(m, "top-right"), H.className = "esri-widget--button", H.title = "Recenter map on selected shot", H.innerHTML = '<span aria-hidden="true" class="esri-icon-zoom-to-object"></span><span class="esri-icon-font-fallback-text">Expand</span>', H.addEventListener("click", () => {
        s.goTo({
          center: [k.geometry.longitude, k.geometry.latitude]
        });
      }), s.ui.add(H, "top-right"), b = t.facing((u) => {
        R(u);
      });
      const j = new URLSearchParams(window.location.hash.substr(1)), z = JSON.parse(j.get("center") || null);
      z && (console.log("got center from hash params", z), s.center = z, _ = !0);
      const w = JSON.parse(j.get("zoom") || null);
      w && (console.log("got zoom from hash params", w), s.zoom = w);
      const M = j.get("marker");
      if (M) {
        const u = JSON.parse(M);
        if (u) {
          const [f, g] = u;
          R(t.facing(), f, g);
        }
      }
      E.watch(s, "scale", ee), ee(s.scale), E.watch(s, "center", Oe), t.shot((u) => {
        const f = parseInt(
          typeof u == "object" && u !== null ? u.id : u
        );
        if (f && f !== de) {
          const g = function() {
            a.forEach((T, L) => {
              const N = T.layer;
              t.resetProgress(), console.log("Querying layer for shot", N, f), N.queryFeatures({
                objectIds: [f],
                returnGeometry: !0,
                outFields: "*",
                where: N.definitionExpression
              }).then((G) => {
                if (console.log("Got results for layer", N, G), G.features.length > 0) {
                  const h = G.features[0];
                  $(h, L);
                }
              });
            });
          };
          if (console.log("Got shot", u, "layers", a.length), a.length === 0) {
            q = g;
            return;
          }
          g();
        } else
          u || t.hide();
      });
    }), D) {
      const m = `${D}/2`, j = new y({
        url: m,
        visible: !0,
        outFields: ["*"],
        editingEnabled: !0,
        popupEnabled: !1,
        renderer: {
          type: "simple",
          symbol: {
            type: "cim",
            data: {
              type: "CIMSymbolReference",
              symbol: {
                type: "CIMPolygonSymbol",
                symbolLayers: [
                  {
                    type: "CIMSolidStroke",
                    enable: !0,
                    colorLocked: !0,
                    capStyle: "Round",
                    joinStyle: "Round",
                    miterLimit: 10,
                    width: 2,
                    color: [255, 255, 255, 255]
                  },
                  {
                    type: "CIMSolidStroke",
                    enable: !0,
                    colorLocked: !0,
                    capStyle: "Round",
                    joinStyle: "Round",
                    miterLimit: 10,
                    width: 4.5,
                    color: [0, 0, 0, 140]
                  }
                ],
                angleAlignment: "Map"
              }
            }
          }
        },
        labelingInfo: null
      });
      s.map.add(j);
      const z = `${D}/0`;
      console.log("shots url is", z);
      const w = new y({
        url: z,
        outFields: ["*"],
        definitionExpression: "mod(id,100) = 0"
        // start with agressive simplifaction - view should get scale change early on to override this
      });
      s.map.add(w), w.when((f) => {
        const g = f.fields, T = g.find((h) => le(h, "filenames")), L = g.find((h) => le(h, "calibration"));
        a.push({
          layer: w,
          shot: "id",
          filenames: "filenames",
          yaw: "yaw",
          rotation: "rotation_matrix",
          datetime: "utc_time",
          brightness: null,
          base: ue(T && T.description),
          calibration: "calibration",
          rigId: null,
          calibrationBase: ue(L.description),
          capture: "capture"
        }), _ ? (console.log("center was set"), _ = !1) : (console.log("center not set using layer extent", f.fullExtent), s.extent = f.fullExtent), ee(), q && (console.log("setting deferred shot", q), q(), q = null);
        const N = (h) => {
          if (!Array.isArray(h) || h.length === 0) return;
          const S = document.createElement("div");
          S.style.padding = "10px", S.style.background = "white", S.style.minWidth = "150px";
          const B = document.createElement("div");
          B.style.marginBottom = "6px", B.style.fontWeight = "bold", B.style.fontSize = "13px", B.textContent = "Symbology", S.appendChild(B), h.forEach((W) => {
            const O = document.createElement("input");
            O.type = "radio", O.name = "geocam-drawing-info", O.value = W.name, O.id = `geocam-di-${W.name}`, O.style.marginRight = "6px", W.name === "Original" && (O.checked = !0, w.renderer = A.fromJSON(W.renderer)), O.addEventListener("change", () => {
              w.renderer = A.fromJSON(W.renderer);
            });
            const C = document.createElement("label");
            C.htmlFor = O.id, C.textContent = W.name, C.style.fontSize = "13px", C.style.cursor = "pointer";
            const J = document.createElement("div");
            J.style.marginBottom = "4px", J.appendChild(O), J.appendChild(C), S.appendChild(J);
          });
          const Se = new Y({
            view: s,
            content: S,
            expandIconClass: "esri-icon-hollow-eye",
            expandTooltip: "Symbology options"
          });
          s.ui.add(Se, "top-right");
        }, G = f.sourceJSON && f.sourceJSON.drawingInfo && f.sourceJSON.drawingInfo.drawingInfoOptions;
        G ? N(G) : fetch(`${z}?f=json`).then((h) => h.json()).then((h) => {
          const S = h.drawingInfo && h.drawingInfo.drawingInfoOptions;
          N(S);
        }).catch((h) => {
          console.warn("Failed to fetch drawingInfoOptions", h);
        });
      });
      const M = `${D}/1`;
      console.log("points features url is", M);
      const u = new y({
        url: M,
        popupEnabled: !0,
        popupTemplate: {
          title: "{reference}",
          content: [
            {
              type: "fields",
              fieldInfos: [
                {
                  fieldName: "embed",
                  label: "content"
                }
              ]
            }
          ]
        }
      });
      s.map.add(u), d = new p({
        title: "GeoCam Field of View",
        geometryType: "point",
        spatialReference: {
          wkid: 4326
        }
      }), s.map.layers.add(d), s.when(() => {
        console.log("mapview when"), s.map.reorder(d, 1e3), s.map.reorder(w, 1e3), s.map.reorder(u, 1e3);
      });
    }
  };
  var fe = function(n) {
    switch (n.key, n.key) {
      case "Escape": {
        I(null), s.graphics.removeAll();
        break;
      }
      case " ":
        o = !0;
    }
  }, ge = function(n) {
    switch (n.key, n.key) {
      case " ":
        o = !1;
    }
  };
  document.addEventListener("keydown", fe), document.addEventListener("keyup", ge), this.destroy = function() {
    document.removeEventListener("keydown", fe), document.removeEventListener("keyup", ge), b(), x(), v(), Te(), Le(), U(), s.map.removeLayer(d), t.wrapper.removeChild(P);
  };
};
class $e extends HTMLElement {
  constructor() {
    super(), this.plugin = null, console.log("GeocamViewerArcgisMap init");
  }
  connectedCallback() {
    const r = this.closest("geocam-viewer");
    if (!r) {
      console.error("GeocamViewerArcgisMap must be a child of GeocamViewer");
      return;
    }
    const t = (o) => {
      const a = r.viewer;
      a && typeof a.plugin == "function" ? o(a) : setTimeout(() => t(o), 50);
    };
    this.link = (o) => {
      console.log("linking to ", o);
      const a = this.getAttribute("src");
      a || console.warn("No src attribute on geocam-viewer-arcgis-map"), t((d) => {
        if (this.plugin)
          return;
        this.viewer = d, this.mapView = o;
        const b = r.querySelector("geocam-viewer-prev-next-control"), x = b && b.plugin;
        this.plugin = new Ke({ mapView: o, prevNextPlugin: x, src: a }), this.viewer.plugin(this.plugin);
        const v = r.querySelector("geocam-viewer-screen-shot");
        v && v.plugin && v.plugin.arcgisView(o);
      });
    }, console.log("GeocamViewerArcgisMap connected");
  }
  disconnectedCallback() {
    this.plugin = null, this.viewer = null, this.mapView = null, console.log("GeocamViewerArcgisMap disconnected");
  }
}
window.customElements.define("geocam-viewer-arcgis-map", $e);
export {
  $e as GeocamViewerArcgisMap
};
