...
const de = {
  Promise: Te ? window.Promise : void 0
};
var me = "4.25", C = "next";
function fe(e) {
  if (e.toLowerCase() === C)
...
function Oe(e, o) {
  ...
  return !o.url && n && (o.url = n), He(o).then(function() {
    return ie(e);
  });
}
const Ye = (e, o) => e.replace(/\(\?\<(.+?)\>[^)]*\)/g, (t, n) => o[n]);
...
const ce = function(e) {
...
}, We = (e, o) => (document.getElementById(e) || document.getElementsByTagName("head")[0].prepend(O("STYLE", { type: "text/css" }, o)), !0), Re = 600, qe = 8, ue = (e, o) => {
  const t = Number(e);
  ...
  We("geocam-argis-map", `
    ...
    `);
  let t, n, a = [], d, g, z, w, j, M, G, Y, J, I, W, he, F, ve, Q = !0, K;
  const { mapView: s, prevNextPlugin: S, widgets: Xe, expands: Ce, src: R } = e, Z = document.createElement("div"), $ = function(r, i, c) {
    return {
      geometry: {
        type: "point",
        latitude: c,
  ...
  const P = function(r, i, c) {
    ...
    ), d.add(L), (c || c === 0) && K([i, c]))) : Z.classList.add("esri-disabled"));
  }, _ = function(r, i, c = {}) {
    ...
    const c = r.base;
    if (r.filenames)
      return JSON.parse(i[r.filenames]).map((u) => Array.isArray(u) ? u.map((b) => /^https?:\/\//i.test(b) ? u : `${c}${b}`) : /^https?:\/\//i.test(u) ? u : `${c}${u}`);
    {
      ...
      return k.map((p, v) => {
        ...
        return `${c}${b}/${v}/${i[r.shot]}.jpg?offset=${y[v]}&length=${p}&container=${A}`;
      });
    }
  }, ye = (r) => {
    if (Array.isArray(r))
    ...
    if (t.label) {
      const { capture: i, utc_time: c, shot: u } = r, b = new Date(c);
  ...
  const V = function(r, i) {
    ...
    const k = [0, 1, 2].map(
      (m) => Ye(u.calibrationBase, {
        camera: m,
        rig_id: r.attributes[u.rigId],
        calibration: r.attributes[u.calibration]
      })
    ), y = r.attributes[u.yaw], h = r.attributes[u.rotation], p = Y() && u.brightness ? r.attributes[u.brightness] : null;
    J = p;
    const v = q(
    ...
    if (c) {
      const m = le(r.geometry, c);
      t.facing(m);
    }
    t.show(l, y, k, h, p, E), P(
      t.facing(),
  ...
  this.init = async function(r) {
    t = r, K = t.store("marker"), I = t.store("zoom"), F = t.store("center"), W = t.store("viewlock"), G = t.store("autorotate"), M = O("DIV", { class: "geocam-auto-rotate" });
    const i = O("LABEL", { class: "geocam-auto-rotate-label" }), c = O("INPUT", {
      type: "checkbox",
    ...
    c.checked = G(), c.addEventListener("change", () => {
      G(c.checked);
    }), i.appendChild(c), i.appendChild(u), M.appendChild(i), t.addControl(M, "left-top"), z = G((h) => {
      M.setAttribute(
        "title",
    ...
    }), Y = t.store("autobrightness"), w = Y((h) => {
      t.reload(h ? J : "[1,1,1]");
    }), j = t.visible((h) => P(t.facing()));
    const [b, k, y] = await Oe([
    ...
    if (s.when(async () => {
      s.on("clickable", (l) => {
        ...
        if (l && l.target && l.target.closest("input,calcite-input"))
          return;
        const m = [
          "ArrowUp",
          "ArrowDown",
        ...
        if (console.log("immediate-click", l, m), n) {
          console.log("space wqas down");
          const f = s.toMap(m);
          if (W(f), B && s.graphics.removeAll(), B = pe(f), s.graphics.add(B), t.visible()) {
            const N = le(L.geometry, f);
            t.facing(N);
          }
        } else
          s.hitTest(m).then((f) => {
            if (f.results && f.results.length > 0)
              for (var N = 0; N < f.results.length; N++) {
                const T = f.results[N].graphic, x = Ae(T);
                if (x >= 0) {
                  Object.entries(T.attributes).length < 2 ? T.layer.queryFeatures({
                    objectIds: [T.attributes.id],
                    returnGeometry: !0,
                    outFields: "*",
                    where: T.layer.definitionExpression
                  }).then((U) => {
                    U.features.length > 0 && V(U.features[0], x);
                  }) : V(T, x);
                  break;
                }
              }
    ...
    <span class="esri-icon-font-fallback-text">Copy short URL to clipboad</span>`, h.addEventListener("click", async () => {
      ...
      const A = JSON.parse(p.get("zoom") || null);
      A && (console.log("got zoom from hash params", A), s.zoom = A);
      ...
      k.watch(s, "scale", oe), oe(s.scale), k.watch(s, "center", ze), t.shot((l) => {
        const m = parseInt(
        ...
        if (m && m !== te) {
          const f = function() {
            a.forEach((N, T) => {
              const x = N.layer;
              t.resetProgress(), console.log("Querying layer for shot", x, m), x.queryFeatures({
                objectIds: [m],
                returnGeometry: !0,
                outFields: "*",
                where: x.definitionExpression
              }).then((U) => {
                if (console.log("Got results for layer", x, U), U.features.length > 0) {
                  const Ne = U.features[0];
                  V(Ne, T);
                }
              });
            });
          };
          if (console.log("Got shot", l, "layers", a.length), a.length === 0) {
            H = f;
            return;
          }
          f();
      ...
      s.map.add(A), A.when((m) => {
        const f = m.fields, N = f.find((x) => _(x, "filenames")), T = f.find((x) => _(x, "calibration"));
        a.push({
          layer: A,
          shot: "id",
          filenames: "filenames",
          ...
          datetime: "utc_time",
          brightness: null,
          base: ee(N && N.description),
          calibration: "calibration",
          rigId: null,
          calibrationBase: ee(T.description),
          capture: "capture"
        }), X ? (console.log("center was set"), X = !1) : (console.log("center not set using layer extent", m.fullExtent), s.extent = m.fullExtent), H && (console.log("setting deferred shot", H), H(), H = null);
      });
      const E = `${R}/1`;
      ...
      s.map.add(l), d = new b({
        ...
        console.log("mapview when"), s.map.reorder(d, 1e3), s.map.reorder(A, 1e3), s.map.reorder(l, 1e3);
      });
    }
  };
  var ne = function(r) {
  ...
  connectedCallback() {
    ...
    this.link = (n) => {
      ...
      a || console.warn("No src attribute on geocam-viewer-arcgis-map"), t((d) => {
        ...
        this.plugin = new Be({ mapView: n, prevNextPlugin: z, src: a }), this.viewer.plugin(this.plugin);
