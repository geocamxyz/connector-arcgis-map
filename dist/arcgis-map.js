...
function Ue(e) {
  ...
  return new de.Promise(function(a, d) {
    ...
    if (g) {
      ...
      if (y) {
        var j = y === !0;
        Me(j ? t : y, o.insertCssBefore);
      }
      g = Pe(n), ae(g, function() {
...
const ce = function(e) {
  ...
  const [t, n] = ce(e), [a, d] = ce(o), g = a.toRad(), z = d.toRad(), y = t.toRad(), j = n.toRad(), M = Math.sin(g - y) * Math.cos(z), G = Math.cos(j) * Math.sin(z) - Math.sin(j) * Math.cos(z) * Math.cos(g - y);
...
}, Se = (e, o) => (document.getElementById(e) || document.getElementsByTagName("head")[0].prepend(U("STYLE", { type: "text/css" }, o)), !0), Re = 600, Ye = 8, ue = (e, o) => {
  const t = Number(e);
  ...
  if (typeof e == "string")
    try {
      ...
      return Array.isArray(t) ? W(t, o) : W([t, t, t], o);
    } catch {
      return [o, o, o];
    }
  if (typeof e == "number") {
  ...
  Se("geocam-argis-map", `
    ...
    `);
  let t, n, a = [], d, g, z, y, j, M, G, H, I, J, O, pe, F, he, Q = !0, K;
  const { mapView: s, prevNextPlugin: q, widgets: Ve, expands: Xe, src: S } = e, P = document.createElement("div"), $ = function(r, i, l) {
    return {
      geometry: {
        type: "point",
        latitude: l,
  ...
  let B = null;
  const Z = function(r, i, l) {
    d && (d.removeAll(), t.visible() ? (P.classList.remove("esri-disabled"), r !== null && (G() ? (s.rotation = r * -1, r = 0, (l || l === 0) && s.goTo({
      center: [i, l]
    })) : s.rotation = 0, L = $(
      r || 0,
      i || L.geometry.longitude,
      l || L.geometry.latitude
    ), d.add(L), (l || l === 0) && K([i, l]))) : P.classList.add("esri-disabled"));
  }, _ = function(r, i, l = {}) {
    ...
    {
      const u = i[r.capture].split(".")[0], b = u.split("/").pop(), k = JSON.parse(i[r.lengths]), w = JSON.parse(i[r.offsets]);
      return k.map((v, h) => {
        const x = encodeURIComponent(
          `https://s3.us-west-004.backblazeb2.com/gc-raw-surveys-archive/${u}_${h}.tar`
        );
        return `${l}${b}/${h}/${i[r.shot]}.jpg?offset=${w[h]}&length=${v}&container=${x}`;
      });
    }
  }, we = (r) => {
    if (Array.isArray(r))
  ...
  const V = function(r, i) {
    ...
    te = b, t.shot(b), q && (q.prev(r.attributes.prev), q.next(r.attributes.next));
    const k = [0, 1, 2].map(
    ...
    ), w = r.attributes[u.yaw], p = r.attributes[u.rotation], v = H() && u.brightness ? r.attributes[u.brightness] : null;
    I = v;
    const h = W(
      ...
      exposure_us: h,
      gain_boost: x,
      tags: we(r.attributes.image_tags)
    }, c = ye(u, r.attributes);
    if (l) {
      const m = le(r.geometry, l);
      t.facing(m);
    }
    t.show(c, w, k, p, v, E), Z(
      t.facing(),
  ...
  const oe = function(r, i, l, u) {
    clearTimeout(re), re = setTimeout(() => {
      const k = Math.ceil(r / 500), w = s.extent, p = `${w.xmin},${w.ymin},${w.xmax},${w.ymax},${w.spatialReference.wkid}`, v = `mod(id,${k}) = 0 AND extent = ${p}`;
      a.forEach((h) => {
        h.layer.definitionExpression !== v && (h.layer.definitionExpression = v, console.log(
  ...
  this.init = async function(r) {
    t = r, K = t.store("marker"), J = t.store("zoom"), F = t.store("center"), O = t.store("viewlock"), G = t.store("autorotate"), M = U("DIV", { class: "geocam-auto-rotate" });
    ...
    l.checked = G(), l.addEventListener("change", () => {
      G(l.checked);
    }), i.appendChild(l), i.appendChild(u), M.appendChild(i), t.addControl(M, "left-top"), z = G((p) => {
      M.setAttribute(
        "title",
    ...
    }), H = t.store("autobrightness"), y = H((p) => {
      t.reload(p ? I : "[1,1,1]");
    }), j = t.visible((p) => Z(t.facing()));
    const [b, k, w] = await He([
    ...
    if (s.when(async () => {
              ...
              for (var T = 0; T < f.results.length; T++) {
                ...
                if (N >= 0) {
                  Object.entries(A.attributes).length < 2 ? A.layer.queryFeatures({
                    ...
                    Y.features.length > 0 && V(Y.features[0], N);
                  }) : V(A, N);
                  break;
                }
              }
      ...
      p.className = "esri-widget--button", p.title = "Copy short URL to clipboad", p.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" height="24" width="24"><path d="M23 15H11.707l2.646 2.646-.707.707L9.793 14.5l3.854-3.854.707.707L11.707 14H23zm-13-5H6v1h4zm-4 5h2v-1H6zM3 4h3V3h3a2 2 0 0 1 4 0h3v1h3v9h-1V5h-2v2H6V5H4v16h14v-5h1v6H3zm4 2h8V4h-3V2.615A.615.615 0 0 0 11.386 2h-.771a.615.615 0 0 0-.615.615V4H7zM6 19h4v-1H6z"></path></svg>
    <span class="esri-icon-font-fallback-text">Copy short URL to clipboad</span>`, p.addEventListener("click", async () => {
      ...
      }), s.ui.add(p, "top-right"), P.className = "esri-widget--button", P.title = "Recenter map on selected shot", P.innerHTML = '<span aria-hidden="true" class="esri-icon-zoom-to-object"></span><span class="esri-icon-font-fallback-text">Expand</span>', P.addEventListener("click", () => {
        s.goTo({
          center: [L.geometry.longitude, L.geometry.latitude]
        });
      }), s.ui.add(P, "top-right"), g = t.facing((c) => {
        Z(c);
      });
      const v = new URLSearchParams(window.location.hash.substr(1)), h = JSON.parse(v.get("center") || null);
      h && (console.log("got center from hash params", h), s.center = h, X = !0);
      const x = JSON.parse(v.get("zoom") || null);
      x && (console.log("got zoom from hash params", x), s.zoom = x);
      const E = v.get("marker");
      if (E) {
      ...
      s.map.add(x), x.when((m) => {
        ...
        a.push({
          ...
          capture: "capture"
        }), X ? (console.log("center was set"), X = !1) : (console.log("center not set using layer extent", m.fullExtent), s.extent = m.fullExtent), R && (t.shot(R), R = null);
      });
      const E = `${S}/1`;
  ...
  document.addEventListener("keydown", ne), document.addEventListener("keyup", se), this.destroy = function() {
    document.removeEventListener("keydown", ne), document.removeEventListener("keyup", se), g(), z(), y(), pe(), he(), j(), s.map.removeLayer(d), t.wrapper.removeChild(M);
  };
};
  ...
  connectedCallback() {
    ...
    this.link = (n) => {
      ...
      a || console.warn("No src attribute on geocam-viewer-arcgis-map"), t((d) => {
        ...
        this.plugin = new qe({ mapView: n, prevNextPlugin: z, src: a }), this.viewer.plugin(this.plugin);
        const y = o.querySelector("geocam-viewer-screen-shot");
        y && y.plugin && y.plugin.arcgisView(n);
      });
