...
function ce(e, n, t) {
  ...
  t && (r = Ge(e, t));
  var s = function() {
    n(e), e.removeEventListener("load", s, !1), r && e.removeEventListener("error", r, !1);
  };
...
function Pe(e) {
  e === void 0 && (e = {});
  var n = {};
  [je, e].forEach(function(s) {
    for (var d in s)
      Object.prototype.hasOwnProperty.call(s, d) && (n[d] = s[d]);
  });
  var t = n.version, r = n.url || ge(t);
  return new de.Promise(function(s, d) {
    var b = he();
    if (b) {
      var A = b.getAttribute("src");
      A !== r ? d(new Error("The ArcGIS API for JavaScript is already loaded (".concat(A, ")."))) : J() ? s(b) : ce(b, s, d);
    } else if (J())
      ...
      if (z) {
        var T = z === !0;
        Me(T ? t : z, n.insertCssBefore);
      }
      b = He(r), ce(b, function() {
        b.setAttribute("data-esri-loader", "loaded"), s(b);
...
const le = function(e) {
  ...
  Ue("geocam-argis-map", `
    ...
    `);
  let t, r, s = [], d, b, A, z, T, E, M, P, Z, I, Y, Q, O, be, F, ve, K = !0, $;
  const { mapView: c, prevNextPlugin: q, widgets: Re, expands: Be, src: C } = e, j = document.createElement("div"), _ = function(o, l, a) {
    return {
      geometry: {
        type: "point",
        latitude: a,
  ...
  let S = null;
  const U = function(o, l, a) {
    d && (d.removeAll(), t.visible() ? (j.classList.remove("esri-disabled"), o !== null && (M() ? (c.rotation = o * -1, o = 0, (a || a === 0) && c.goTo({
      center: [l, a]
    })) : c.rotation = 0, L = _(
      o || 0,
      l || L.geometry.longitude,
      a || L.geometry.latitude
    ), d.add(L), (a || a === 0) && $([l, a]))) : j.classList.add("esri-disabled"));
  }, ee = function(o, l, a = {}) {
    const i = new RegExp(l, "i");
    let f = i.test(o.name) || i.test(o.alias);
    return f && a.description && (f = !!o.description), f;
  }, te = function(o) {
    if (o) {
    ...
    const a = o.base;
    if (o.filenames)
      return JSON.parse(l[o.filenames]).map((i) => Array.isArray(i) ? i.map((f) => /^https?:\/\//i.test(f) ? i : `${a}${f}`) : /^https?:\/\//i.test(i) ? i : `${a}${i}`);
    {
      const i = l[o.capture].split(".")[0], f = i.split("/").pop(), N = JSON.parse(l[o.lengths]), p = JSON.parse(l[o.offsets]);
      return N.map((w, m) => {
        const v = encodeURIComponent(
          `https://s3.us-west-004.backblazeb2.com/gc-raw-surveys-archive/${i}_${m}.tar`
        );
        return `${a}${f}/${m}/${l[o.shot]}.jpg?offset=${p[m]}&length=${w}&container=${v}`;
      });
    }
  }, xe = function(o) {
    return s.findIndex((l) => l.layer == o.layer);
  }, ye = function(o) {
    if (t.label) {
      const { capture: l, utc_time: a, shot: i } = o, f = new Date(a);
  ...
  const X = function(o, l) {
    const a = O();
    console.log("shotclick with viewlock", a);
    const i = s[l], f = o.attributes[i.shot];
    oe = f, t.shot(f), q && (q.prev(o.attributes.prev), q.next(o.attributes.next));
    const N = [0, 1, 2].map(
      (v) => Ye(i.calibrationBase, {
        camera: v,
        rig_id: o.attributes[i.rigId],
        calibration: o.attributes[i.calibration]
      })
    ), p = o.attributes[i.yaw], H = o.attributes[i.rotation], w = Z() && i.brightness ? o.attributes[i.brightness] : null;
    I = w;
    const m = we(i, o.attributes);
    if (a) {
      const v = ue(o.geometry, a);
      t.facing(v);
    }
    t.show(m, p, N, H, w), U(
      t.facing(),
  ...
  let ne;
  const re = function(o, l, a, i) {
    clearTimeout(ne), ne = setTimeout(() => {
      const N = Math.ceil(o / 500), p = c.extent, H = `${p.xmin},${p.ymin},${p.xmax},${p.ymax},${p.spatialReference.wkid}`, w = `mod(id,${N}) = 0 AND extent = ${H}`;
      s.forEach((m) => {
    ...
    F([c.center.longitude, c.center.latitude]);
  };
  this.init = async function(o) {
    t = o, $ = t.store("marker"), Q = t.store("zoom"), F = t.store("center"), O = t.store("viewlock"), M = t.store("autorotate"), E = k("DIV", { class: "geocam-auto-rotate" });
    const l = k("LABEL", { class: "geocam-auto-rotate-label" }), a = k("INPUT", {
      type: "checkbox",
    ...
    a.checked = M(), a.addEventListener("change", () => {
      M(a.checked);
    }), l.appendChild(a), l.appendChild(i), E.appendChild(l), t.addControl(E, "left-top"), A = M((m) => {
      E.setAttribute(
        "title",
    ...
    }), Z = t.store("autobrightness"), P = k("DIV", { class: "geocam-auto-brightness" });
    ...
    Y.disabled = !0, Y.checked = Z(), Y.addEventListener("change", () => {
      Z(Y.checked);
    }), f.appendChild(Y), f.appendChild(N), P.appendChild(f), t.addControl(P, "left-top"), z = Z((m) => {
      P.setAttribute(
        "title",
        m ? "turn auto-brightness off" : "turn auto-brightness on"
      ), t.reload(Z() ? I : "[1,1,1]");
    }), T = t.visible((m) => U(t.facing())), c.when(async () => {
      c.on("clickable", (u) => {
        K = u;
      }), c.on("key-down", (u) => {
        const h = [
        ...
        t.visible() && h.indexOf(g) !== -1 && u.stopPropagation();
      }), c.on("immediate-click", (u) => {
        if (!K)
          return;
        const h = {
          x: u.x,
        ...
        if (console.log("immediate-click", u, h), r) {
          console.log("space wqas down");
          const g = c.toMap(h);
          if (O(g), S && c.graphics.removeAll(), S = pe(g), c.graphics.add(S), t.visible()) {
            const x = ue(L.geometry, g);
            t.facing(x);
          }
        } else
          c.hitTest(h).then((g) => {
            if (g.results && g.results.length > 0)
              for (var x = 0; x < g.results.length; x++) {
                const y = g.results[x].graphic, G = xe(y);
                if (G >= 0) {
                  Object.entries(y.attributes).length < 2 ? y.layer.queryFeatures({
                    ...
                    B.features.length > 0 && X(B.features[0], G);
                  }) : X(y, G);
                  break;
                }
              }
    ...
    <span class="esri-icon-font-fallback-text">Copy short URL to clipboad</span>`, m.addEventListener("click", async () => {
        ...
        try {
          ...
          alert(
            `Sorry, the short URL could not be copied to the clipboard: ${h}`
          );
        }
      }), c.ui.add(m, "top-right"), j.className = "esri-widget--button", j.title = "Recenter map on selected shot", j.innerHTML = '<span aria-hidden="true" class="esri-icon-zoom-to-object"></span><span class="esri-icon-font-fallback-text">Expand</span>', j.addEventListener("click", () => {
        c.goTo({
          center: [L.geometry.longitude, L.geometry.latitude]
        });
      }), c.ui.add(j, "top-right"), b = t.facing((u) => {
        U(u);
      });
      const v = new URLSearchParams(window.location.hash.substr(1)), W = v.get("center");
      W && (c.center = JSON.parse(W));
      const V = v.get("zoom");
      V && (c.zoom = JSON.parse(V));
      const R = v.get("marker");
      if (R) {
      ...
      H.watch(c, "scale", re), re(c.scale), H.watch(c, "center", Ae), t.shot((u) => {
        const h = parseInt(
          typeof u == "object" && u !== null ? u.id : u
        );
        h && h !== oe ? (console.log("Got shot", u, "layers", s.length), s.forEach((g, x) => {
          const y = g.layer;
          t.resetProgress(), console.log("Querying layer for shot", y, h), y.queryFeatures({
            objectIds: [h],
            returnGeometry: !0,
            outFields: "*",
            where: y.definitionExpression
          }).then((G) => {
            if (console.log("Got results for layer", y, G), G.features.length > 0) {
              const B = G.features[0];
              X(B, x);
            }
          });
        })) : u || t.hide();
    ...
    if (C) {
      ...
      c.map.add(v), v.when((R) => {
        const u = R.fields, h = u.find((x) => ee(x, "filenames")), g = u.find(
          (x) => ee(x, "calibration")
        );
        s.push({
          layer: v,
          ...
          datetime: "utc_time",
          brightness: null,
          base: te(h && h.description),
          calibration: "calibration",
          rigId: null,
          calibrationBase: te(g.description),
          capture: "capture"
        });
      });
      const W = `${C}/1`;
  ...
  document.addEventListener("keydown", ae), document.addEventListener("keyup", se), this.destroy = function() {
    document.removeEventListener("keydown", ae), document.removeEventListener("keyup", se), b(), A(), z(), be(), ve(), T(), c.map.removeLayer(d), t.wrapper.removeChild(E), t.wrapper.removeChild(P);
  };
};
class Ve extends HTMLElement {
  ...
  connectedCallback() {
    this.link = function(n) {
      ...
      if (r.viewer && r.viewer.plugin) {
        const s = document.getElementsByTagName(
          "geocam-viewer-prev-next-control"
        )[0], d = s && s.plugin;
        this.plugin = new Oe({ mapView: n, prevNextPlugin: d, src: t }), r.viewer.plugin(this.plugin);
      } else
        console.error(
  ...
  disconnectedCallback() {
    this.plugin = null, console.log("GeocamViewerArcgisMap disconnected");
  }
}
window.customElements.define("geocam-viewer-arcgis-map", Ve);
export {
  Ve as GeocamViewerArcgisMap
};
