...
const toDefaultedNumber = (value, fallback) => {
...
const normalizeTriplet = (value, fallback) => {
  if (Array.isArray(value)) {
    return [0, 1, 2].map((idx) => toDefaultedNumber(value[idx], fallback));
  }
  ...
  if (typeof value === "string") {
    try {
      ...
      if (Array.isArray(parsed)) {
        return normalizeTriplet(parsed, fallback);
      }
      return normalizeTriplet([parsed, parsed, parsed], fallback);
    } catch (err) {
  ...
  if (typeof value === "number") {
    const numeric = toDefaultedNumber(value, fallback);
...
export const arcgisMap = function (config = {}) {
  ...
  const shotClick = function (graphic, shotLayerIndex) {
    ...
    const shotInfo = {
      id: shotIndex,
      latitude:
        graphic.geometry && typeof graphic.geometry.latitude === "number"
          ? graphic.geometry.latitude
          : null,
      longitude:
        graphic.geometry && typeof graphic.geometry.longitude === "number"
          ? graphic.geometry.longitude
          : null,
      utc_time: graphic.attributes.utc_time || null,
    ...
    viewer.show(urls, yaw, hemispheres, rotation, brightness, shotInfo);
    updateFov(
      viewer.facing(),
      graphic.geometry.longitude,
      graphic.geometry.latitude
