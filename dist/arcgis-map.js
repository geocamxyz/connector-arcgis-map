var xe = typeof window < "u";
const le = {
  Promise: xe ? window.Promise : void 0
};
var ue = "4.25", V = "next";
function de(e) {
  if (e.toLowerCase() === V)
    return V;
  var n = e && e.match(/^(\d)\.(\d+)/);
  return n && {
    major: parseInt(n[1], 10),
    minor: parseInt(n[2], 10)
  };
}
function me(e) {
  return e === void 0 && (e = ue), "https://js.arcgis.com/".concat(e, "/");
}
function ze(e) {
  e === void 0 && (e = ue);
  var n = me(e), t = de(e);
  if (t !== V && t.major === 3) {
    var o = t.minor <= 10 ? "js/" : "";
    return "".concat(n).concat(o, "esri/css/esri.css");
  } else
    return "".concat(n, "esri/themes/light/main.css");
}
function Ne(e) {
  var n = document.createElement("link");
  return n.rel = "stylesheet", n.href = e, n;
}
function Te(e, n) {
  if (n) {
    var t = document.querySelector(n);
    t.parentNode.insertBefore(e, t);
  } else
    document.head.appendChild(e);
}
function Le(e) {
  return document.querySelector('link[href*="'.concat(e, '"]'));
}
function ke(e) {
  return !e || de(e) ? ze(e) : e;
}
function Ee(e, n) {
  var t = ke(e), o = Le(t);
  return o || (o = Ne(t), Te(o, n)), o;
}
var je = {};
function Me(e) {
  var n = document.createElement("script");
  return n.type = "text/javascript", n.src = e, n.setAttribute("data-esri-loader", "loading"), n;
}
function oe(e, n, t) {
  var o;
  t && (o = Ge(e, t));
  var i = function() {
    n(e), e.removeEventListener("load", i, !1), o && e.removeEventListener("error", o, !1);
  };
  e.addEventListener("load", i, !1);
}
function Ge(e, n) {
  var t = function(o) {
    n(o.error || new Error("There was an error attempting to load ".concat(e.src))), e.removeEventListener("error", t, !1);
  };
  return e.addEventListener("error", t, !1), t;
}
function fe() {
  return document.querySelector("script[data-esri-loader]");
}
function X() {
  var e = window.require;
  return e && e.on;
}
function Ze(e) {
  e === void 0 && (e = {});
  var n = {};
  [je, e].forEach(function(i) {
    for (var d in i)
      Object.prototype.hasOwnProperty.call(i, d) && (n[d] = i[d]);
  });
  var t = n.version, o = n.url || me(t);
  return new le.Promise(function(i, d) {
    var g = fe();
    if (g) {
      var x = g.getAttribute("src");
      x !== o ? d(new Error("The ArcGIS API for JavaScript is already loaded (".concat(x, ")."))) : X() ? i(g) : oe(g, i, d);
    } else if (X())
      d(new Error("The ArcGIS API for JavaScript is already loaded."));
    else {
      var w = n.css;
      if (w) {
        var j = w === !0;
        Ee(j ? t : w, n.insertCssBefore);
      }
      g = Me(o), oe(g, function() {
        g.setAttribute("data-esri-loader", "loaded"), i(g);
      }, d), document.body.appendChild(g);
    }
  });
}
function se(e) {
  return new le.Promise(function(n, t) {
    var o = window.require.on("error", t);
    window.require(e, function() {
      for (var i = [], d = 0; d < arguments.length; d++)
        i[d] = arguments[d];
      o.remove(), n(i);
    });
  });
}
function Pe(e, n) {
  if (n === void 0 && (n = {}), X())
    return se(e);
  var t = fe(), o = t && t.getAttribute("src");
  return !n.url && o && (n.url = o), Ze(n).then(function() {
    return se(e);
  });
}
const Ue = (e, n) => e.replace(/\(\?\<(.+?)\>[^)]*\)/g, (t, o) => n[o]);
Number.prototype.toRad = function() {
  return this * (Math.PI / 180);
};
Number.prototype.toDeg = function() {
  return this * (180 / Math.PI);
};
Number.prototype.to2DP = function() {
  return parseFloat(this.toFixed(2));
};
const ae = function(e) {
  return Array.isArray(e) ? e : [
    e.longitude || (e.coords ? e.coords.longitude : e.geometry ? e.geometry.longitude : null),
    e.latitude || (e.coords ? e.coords.latitude : e.geometry ? e.geometry.latitude : null)
  ];
}, ie = function(e, n) {
  const [t, o] = ae(e), [i, d] = ae(n), g = i.toRad(), x = d.toRad(), w = t.toRad(), j = o.toRad(), M = Math.sin(g - w) * Math.cos(x), G = Math.cos(j) * Math.sin(x) - Math.sin(j) * Math.cos(x) * Math.cos(g - w);
  return (Math.atan2(M, G).toDeg() + 360) % 360;
}, U = (e, n = {}, t = "") => {
  const o = document.createElement(e);
  for (let i in n)
    o.setAttribute(i, n[i]);
  return o.innerHTML = t, o;
}, He = (e, n) => (document.getElementById(e) || document.getElementsByTagName("head")[0].prepend(U("STYLE", { type: "text/css" }, n)), !0), Oe = 600, Ye = 8, ce = (e, n) => {
  const t = Number(e);
  return Number.isFinite(t) && t > 0 ? t : n;
}, R = (e, n) => {
  if (Array.isArray(e))
    return [0, 1, 2].map((t) => ce(e[t], n));
  if (typeof e == "string")
    try {
      const t = JSON.parse(e);
      return Array.isArray(t) ? R(t, n) : R([t, t, t], n);
    } catch {
      return [n, n, n];
    }
  if (typeof e == "number") {
    const t = ce(e, n);
    return [t, t, t];
  }
  return [n, n, n];
}, We = "data:image/png;base64,", Re = function(e = {}) {
  He("geocam-argis-map", `
      .geocam-auto-rotate-checkbox {
        display: none;
      }

      .geocam-auto-rotate-span {
        background-image: url("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' enable-background='new 0 0 24 24'  viewBox='0 0 24 24' fill='currentColor'><rect fill='none' height='24' width='24'/><path d='M7.94,5.12L6.49,3.66C8.07,2.61,9.96,2,12,2c5.52,0,10,4.48,10,10c0,2.04-0.61,3.93-1.66,5.51l-1.46-1.46 C19.59,14.86,20,13.48,20,12c0-4.41-3.59-8-8-8C10.52,4,9.14,4.41,7.94,5.12z M17.66,9.53l-1.41-1.41l-2.65,2.65l1.41,1.41 L17.66,9.53z M19.78,22.61l-2.27-2.27C15.93,21.39,14.04,22,12,22C6.48,22,2,17.52,2,12c0-2.04,0.61-3.93,1.66-5.51L1.39,4.22 l1.41-1.41l18.38,18.38L19.78,22.61z M16.06,18.88l-3.88-3.88l-1.59,1.59l-4.24-4.24l1.41-1.41l2.83,2.83l0.18-0.18L5.12,7.94 C4.41,9.14,4,10.52,4,12c0,4.41,3.59,8,8,8C13.48,20,14.86,19.59,16.06,18.88z'/></svg>")
      }

      .geocam-auto-rotate-checkbox:checked +.geocam-auto-rotate-span {
          background-image: url("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' enable-background='new 0 0 24 24' viewBox='0 0 24 24' fill='currentColor'><rect fill='none' height='24' width='24'/><path d='M18.6,19.5H21v2h-6v-6h2v2.73c1.83-1.47,3-3.71,3-6.23c0-4.07-3.06-7.44-7-7.93V2.05c5.05,0.5,9,4.76,9,9.95 C22,14.99,20.68,17.67,18.6,19.5z M4,12c0-2.52,1.17-4.77,3-6.23V8.5h2v-6H3v2h2.4C3.32,6.33,2,9.01,2,12c0,5.19,3.95,9.45,9,9.95 v-2.02C7.06,19.44,4,16.07,4,12z M16.24,8.11l-5.66,5.66l-2.83-2.83l-1.41,1.41l4.24,4.24l7.07-7.07L16.24,8.11z'/></svg>")
      }

    `);
  let t, o, i = [], d, g, x, w, j, M, G, H, C, D, O, ge, J, be, I = !0, F;
  const { mapView: s, prevNextPlugin: q, widgets: Se, expands: Be, src: Y } = e, Z = document.createElement("div"), Q = function(r, a, c) {
    return {
      geometry: {
        type: "point",
        latitude: c,
        longitude: a
      },
      symbol: {
        type: "picture-marker",
        // autocasts as new PictureMarkerSymbol()
        url: We,
        width: "56",
        height: "101",
        angle: r
      }
    };
  };
  let L = Q(0, 0, 0);
  const ve = function(r) {
    return {
      attributes: {
        id: "lock"
      },
      geometry: {
        type: "point",
        latitude: r.latitude,
        longitude: r.longitude
      },
      symbol: {
        type: "simple-marker",
        // autocasts as new SimpleMarkerSymbol()
        path: "M12 4C7.31 4 3.07 5.9 0 8.98L12 21l5-5.01V8h5.92C19.97 5.51 16.16 4 12 4zm7 14h2v2h-2z M19 10h2v6h-2z",
        color: [255, 0, 0, 0.8],
        //"rgba(0,0,0,0)", for some reason if color isn't changed then outline colour doesn't change either.
        size: 24,
        // pixels
        xoffset: 0,
        yoffset: 12,
        outline: {
          // autocasts as new SimpleLineSymbol()
          color: [255, 255, 255, 1],
          width: 0
          // points
        }
      }
    };
  };
  let S = null;
  const P = function(r, a, c) {
    d && (d.removeAll(), t.visible() ? (Z.classList.remove("esri-disabled"), r !== null && (G() ? (s.rotation = r * -1, r = 0, (c || c === 0) && s.goTo({
      center: [a, c]
    })) : s.rotation = 0, L = Q(
      r || 0,
      a || L.geometry.longitude,
      c || L.geometry.latitude
    ), d.add(L), (c || c === 0) && F([a, c]))) : Z.classList.add("esri-disabled"));
  }, K = function(r, a, c = {}) {
    const u = new RegExp(a, "i");
    let b = u.test(r.name) || u.test(r.alias);
    return b && c.description && (b = !!r.description), b;
  }, $ = function(r) {
    if (r) {
      var a = document.createElement("textarea");
      return a.innerHTML = r, a.value;
    } else
      return "";
  }, pe = (r, a) => {
    const c = r.base;
    if (r.filenames)
      return JSON.parse(a[r.filenames]).map((u) => Array.isArray(u) ? u.map((b) => /^https?:\/\//i.test(b) ? u : `${c}${b}`) : /^https?:\/\//i.test(u) ? u : `${c}${u}`);
    {
      const u = a[r.capture].split(".")[0], b = u.split("/").pop(), k = JSON.parse(a[r.lengths]), y = JSON.parse(a[r.offsets]);
      return k.map((h, p) => {
        const z = encodeURIComponent(
          `https://s3.us-west-004.backblazeb2.com/gc-raw-surveys-archive/${u}_${p}.tar`
        );
        return `${c}${b}/${p}/${a[r.shot]}.jpg?offset=${y[p]}&length=${h}&container=${z}`;
      });
    }
  }, he = (r) => {
    if (Array.isArray(r)) return r.filter((a) => typeof a == "string");
    if (typeof r == "string")
      try {
        const a = JSON.parse(r);
        if (Array.isArray(a))
          return a.filter((c) => typeof c == "string");
      } catch {
        return [];
      }
    return [];
  }, we = function(r) {
    return i.findIndex((a) => a.layer == r.layer);
  }, ye = function(r) {
    if (t.label) {
      const { capture: a, utc_time: c, shot: u } = r, b = new Date(c);
      t.label(`${b.toLocaleString()}`);
    }
  };
  let _;
  const B = function(r, a) {
    const c = O();
    console.log("shotclick with viewlock", c);
    const u = i[a], b = r.attributes[u.shot];
    _ = b, t.shot(b), q && (q.prev(r.attributes.prev), q.next(r.attributes.next));
    const k = [0, 1, 2].map(
      (m) => Ue(u.calibrationBase, {
        camera: m,
        rig_id: r.attributes[u.rigId],
        calibration: r.attributes[u.calibration]
      })
    ), y = r.attributes[u.yaw], v = r.attributes[u.rotation], h = H() && u.brightness ? r.attributes[u.brightness] : null;
    C = h;
    const p = R(
      r.attributes.exposure_us,
      Oe
    ), z = R(
      r.attributes.gain_boost,
      Ye
    ), E = {
      id: b,
      latitude: r.geometry && typeof r.geometry.latitude == "number" ? r.geometry.latitude : null,
      longitude: r.geometry && typeof r.geometry.longitude == "number" ? r.geometry.longitude : null,
      utc_time: r.attributes.utc_time || null,
      capture: r.attributes[u.capture] || null,
      exposure_us: p,
      gain_boost: z,
      tags: he(r.attributes.image_tags)
    }, l = pe(u, r.attributes);
    if (c) {
      const m = ie(r.geometry, c);
      t.facing(m);
    }
    t.show(l, y, k, v, h, E), P(
      t.facing(),
      r.geometry.longitude,
      r.geometry.latitude
    ), ye(r.attributes);
  };
  let ee;
  const te = function(r, a, c, u) {
    clearTimeout(ee), ee = setTimeout(() => {
      const k = Math.ceil(r / 500), y = s.extent, v = `${y.xmin},${y.ymin},${y.xmax},${y.ymax},${y.spatialReference.wkid}`, h = `mod(id,${k}) = 0 AND extent = ${v}`;
      i.forEach((p) => {
        p.layer.definitionExpression !== h && (p.layer.definitionExpression = h, console.log(
          "definition expression changed for",
          p.layer,
          h
        ));
      }), D(s.zoom), P(t.facing());
    }, 500);
  }, Ae = function(r, a, c, u) {
    J([s.center.longitude, s.center.latitude]);
  };
  this.init = async function(r) {
    t = r, F = t.store("marker"), D = t.store("zoom"), J = t.store("center"), O = t.store("viewlock"), G = t.store("autorotate"), M = U("DIV", { class: "geocam-auto-rotate" });
    const a = U("LABEL", { class: "geocam-auto-rotate-label" }), c = U("INPUT", {
      type: "checkbox",
      class: "geocam-auto-rotate-checkbox"
    }), u = U(
      "SPAN",
      { class: "geocam-auto-rotate-span geocam-viewer-control-button" },
      " Autorotate"
    );
    c.checked = G(), c.addEventListener("change", () => {
      G(c.checked);
    }), a.appendChild(c), a.appendChild(u), M.appendChild(a), t.addControl(M, "left-top"), x = G((v) => {
      M.setAttribute(
        "title",
        v ? "turn auto-rotate off" : "turn auto-rotate on"
      ), P(
        t.facing(),
        L.geometry.longitude,
        L.geometry.latitude
      );
    }), H = t.store("autobrightness"), w = H((v) => {
      t.reload(v ? C : "[1,1,1]");
    }), j = t.visible((v) => P(t.facing()));
    const [b, k, y] = await Pe([
      "esri/layers/GraphicsLayer",
      "esri/core/watchUtils",
      "esri/layers/FeatureLayer"
    ]);
    if (s.when(async () => {
      s.on("clickable", (l) => {
        I = l;
      }), s.on("key-down", (l) => {
        if (l && l.target && l.target.closest("input,calcite-input")) return;
        const m = [
          "ArrowUp",
          "ArrowDown",
          "ArrowRight",
          "ArrowLeft",
          "a",
          "d",
          "w",
          "s"
          // w and s don't seem to be used for map actions but just in case that changes in the future.
        ], f = l && l.key;
        t.visible() && m.indexOf(f) !== -1 && l.stopPropagation();
      }), s.on("immediate-click", (l) => {
        if (!I) return;
        const m = {
          x: l.x,
          y: l.y
        };
        if (console.log("immediate-click", l, m), o) {
          console.log("space wqas down");
          const f = s.toMap(m);
          if (O(f), S && s.graphics.removeAll(), S = ve(f), s.graphics.add(S), t.visible()) {
            const T = ie(L.geometry, f);
            t.facing(T);
          }
        } else
          s.hitTest(m).then((f) => {
            if (f.results && f.results.length > 0)
              for (var T = 0; T < f.results.length; T++) {
                const A = f.results[T].graphic, N = we(A);
                if (N >= 0) {
                  Object.entries(A.attributes).length < 2 ? A.layer.queryFeatures({
                    objectIds: [A.attributes.id],
                    returnGeometry: !0,
                    outFields: "*",
                    where: A.layer.definitionExpression
                  }).then((W) => {
                    W.features.length > 0 && B(W.features[0], N);
                  }) : B(A, N);
                  break;
                }
              }
          });
      });
      const v = document.createElement("div");
      v.className = "esri-widget--button", v.title = "Copy short URL to clipboad", v.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" height="24" width="24"><path d="M23 15H11.707l2.646 2.646-.707.707L9.793 14.5l3.854-3.854.707.707L11.707 14H23zm-13-5H6v1h4zm-4 5h2v-1H6zM3 4h3V3h3a2 2 0 0 1 4 0h3v1h3v9h-1V5h-2v2H6V5H4v16h14v-5h1v6H3zm4 2h8V4h-3V2.615A.615.615 0 0 0 11.386 2h-.771a.615.615 0 0 0-.615.615V4H7zM6 19h4v-1H6z"></path></svg>
    <span class="esri-icon-font-fallback-text">Copy short URL to clipboad</span>`, v.addEventListener("click", async () => {
        const l = `${document.location.origin}/🔗`;
        try {
          const f = await (await fetch(l, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              link: { url: document.location.toString() }
            })
          })).json();
          await navigator.clipboard.writeText(f.link), alert(`Url copied to clipboard: ${f.link}`);
        } catch (m) {
          alert(
            `Sorry, the short URL could not be copied to the clipboard: ${m}`
          );
        }
      }), s.ui.add(v, "top-right"), Z.className = "esri-widget--button", Z.title = "Recenter map on selected shot", Z.innerHTML = '<span aria-hidden="true" class="esri-icon-zoom-to-object"></span><span class="esri-icon-font-fallback-text">Expand</span>', Z.addEventListener("click", () => {
        s.goTo({
          center: [L.geometry.longitude, L.geometry.latitude]
        });
      }), s.ui.add(Z, "top-right"), g = t.facing((l) => {
        P(l);
      });
      const h = new URLSearchParams(window.location.hash.substr(1)), p = h.get("center");
      p && (s.center = JSON.parse(p));
      const z = h.get("zoom");
      z && (s.zoom = JSON.parse(z));
      const E = h.get("marker");
      if (E) {
        const l = JSON.parse(E);
        if (l) {
          const [m, f] = l;
          P(t.facing(), m, f);
        }
      }
      k.watch(s, "scale", te), te(s.scale), k.watch(s, "center", Ae), t.shot((l) => {
        const m = parseInt(
          typeof l == "object" && l !== null ? l.id : l
        );
        m && m !== _ ? (console.log("Got shot", l, "layers", i.length), i.forEach((f, T) => {
          const A = f.layer;
          t.resetProgress(), console.log("Querying layer for shot", A, m), A.queryFeatures({
            objectIds: [m],
            returnGeometry: !0,
            outFields: "*",
            where: A.definitionExpression
          }).then((N) => {
            if (console.log("Got results for layer", A, N), N.features.length > 0) {
              const W = N.features[0];
              B(W, T);
            }
          });
        })) : l || t.hide();
      });
    }), Y) {
      const v = `${Y}/2`, h = new y({
        url: v,
        visible: !1,
        outFields: ["*"],
        editingEnabled: !0
      });
      s.map.add(h);
      const p = `${Y}/0`;
      console.log("shots url is", p);
      const z = new y({
        url: p,
        definitionExpression: "mod(id,100) = 0"
        // start with agressive simplifaction - view should get scale change early on to override this
      });
      s.map.add(z), z.when((m) => {
        const f = m.fields, T = f.find((N) => K(N, "filenames")), A = f.find((N) => K(N, "calibration"));
        i.push({
          layer: z,
          shot: "id",
          filenames: "filenames",
          yaw: "yaw",
          rotation: "rotation_matrix",
          datetime: "utc_time",
          brightness: null,
          base: $(T && T.description),
          calibration: "calibration",
          rigId: null,
          calibrationBase: $(A.description),
          capture: "capture"
        }), s.extent = m.fullExtent;
      });
      const E = `${Y}/1`;
      console.log("points features url is", E);
      const l = new y({
        url: E,
        popupEnabled: !0,
        popupTemplate: {
          title: "{reference}",
          content: [
            {
              type: "fields",
              fieldInfos: [
                {
                  fieldName: "embed",
                  label: "content"
                }
              ]
            }
          ]
        }
      });
      s.map.add(l), d = new b({
        title: "GeoCam Field of View",
        geometryType: "point",
        spatialReference: {
          wkid: 4326
        }
      }), s.map.layers.add(d), s.when(() => {
        s.map.reorder(d, 1e3), s.map.reorder(z, 1e3), s.map.reorder(l, 1e3);
      });
    }
  };
  var re = function(r) {
    switch (r.key, r.key) {
      case "Escape": {
        O(null), s.graphics.removeAll();
        break;
      }
      case " ":
        o = !0;
    }
  }, ne = function(r) {
    switch (r.key, r.key) {
      case " ":
        o = !1;
    }
  };
  document.addEventListener("keydown", re), document.addEventListener("keyup", ne), this.destroy = function() {
    document.removeEventListener("keydown", re), document.removeEventListener("keyup", ne), g(), x(), w(), ge(), be(), j(), s.map.removeLayer(d), t.wrapper.removeChild(M);
  };
};
class qe extends HTMLElement {
  constructor() {
    super(), this.plugin = null, console.log("GeocamViewerArcgisMap init");
  }
  connectedCallback() {
    const n = this.closest("geocam-viewer");
    if (!n) {
      console.error("GeocamViewerArcgisMap must be a child of GeocamViewer");
      return;
    }
    const t = (o) => {
      const i = n.viewer;
      i && typeof i.plugin == "function" ? o(i) : setTimeout(() => t(o), 50);
    };
    this.link = (o) => {
      console.log("linking to ", o);
      const i = this.getAttribute("src");
      i || console.warn("No src attribute on geocam-viewer-arcgis-map"), t((d) => {
        if (this.plugin)
          return;
        this.viewer = d, this.mapView = o;
        const g = n.querySelector("geocam-viewer-prev-next-control"), x = g && g.plugin;
        this.plugin = new Re({ mapView: o, prevNextPlugin: x, src: i }), this.viewer.plugin(this.plugin);
        const w = n.querySelector("geocam-viewer-screen-shot");
        w && w.plugin && w.plugin.arcgisView(o);
      });
    }, console.log("GeocamViewerArcgisMap connected");
  }
  disconnectedCallback() {
    this.plugin = null, this.viewer = null, this.mapView = null, console.log("GeocamViewerArcgisMap disconnected");
  }
}
window.customElements.define("geocam-viewer-arcgis-map", qe);
export {
  qe as GeocamViewerArcgisMap
};
