var Ze = typeof window < "u";
const ye = {
  Promise: Ze ? window.Promise : void 0
};
var xe = "4.25", ee = "next";
function Ae(e) {
  if (e.toLowerCase() === ee)
    return ee;
  var r = e && e.match(/^(\d)\.(\d+)/);
  return r && {
    major: parseInt(r[1], 10),
    minor: parseInt(r[2], 10)
  };
}
function Ne(e) {
  return e === void 0 && (e = xe), "https://js.arcgis.com/".concat(e, "/");
}
function Pe(e) {
  e === void 0 && (e = xe);
  var r = Ne(e), t = Ae(e);
  if (t !== ee && t.major === 3) {
    var o = t.minor <= 10 ? "js/" : "";
    return "".concat(r).concat(o, "esri/css/esri.css");
  } else
    return "".concat(r, "esri/themes/light/main.css");
}
function He(e) {
  var r = document.createElement("link");
  return r.rel = "stylesheet", r.href = e, r;
}
function Se(e, r) {
  if (r) {
    var t = document.querySelector(r);
    t.parentNode.insertBefore(e, t);
  } else
    document.head.appendChild(e);
}
function Ye(e) {
  return document.querySelector('link[href*="'.concat(e, '"]'));
}
function We(e) {
  return !e || Ae(e) ? Pe(e) : e;
}
function Re(e, r) {
  var t = We(e), o = Ye(t);
  return o || (o = He(t), Se(o, r)), o;
}
var qe = {};
function Be(e) {
  var r = document.createElement("script");
  return r.type = "text/javascript", r.src = e, r.setAttribute("data-esri-loader", "loading"), r;
}
function pe(e, r, t) {
  var o;
  t && (o = Ve(e, t));
  var a = function() {
    r(e), e.removeEventListener("load", a, !1), o && e.removeEventListener("error", o, !1);
  };
  e.addEventListener("load", a, !1);
}
function Ve(e, r) {
  var t = function(o) {
    r(o.error || new Error("There was an error attempting to load ".concat(e.src))), e.removeEventListener("error", t, !1);
  };
  return e.addEventListener("error", t, !1), t;
}
function ze() {
  return document.querySelector("script[data-esri-loader]");
}
function te() {
  var e = window.require;
  return e && e.on;
}
function Ce(e) {
  e === void 0 && (e = {});
  var r = {};
  [qe, e].forEach(function(a) {
    for (var d in a)
      Object.prototype.hasOwnProperty.call(a, d) && (r[d] = a[d]);
  });
  var t = r.version, o = r.url || Ne(t);
  return new ye.Promise(function(a, d) {
    var p = ze();
    if (p) {
      var x = p.getAttribute("src");
      x !== o ? d(new Error("The ArcGIS API for JavaScript is already loaded (".concat(x, ")."))) : te() ? a(p) : pe(p, a, d);
    } else if (te())
      d(new Error("The ArcGIS API for JavaScript is already loaded."));
    else {
      var v = r.css;
      if (v) {
        var Z = v === !0;
        Re(Z ? t : v, r.insertCssBefore);
      }
      p = Be(o), pe(p, function() {
        p.setAttribute("data-esri-loader", "loaded"), a(p);
      }, d), document.body.appendChild(p);
    }
  });
}
function he(e) {
  return new ye.Promise(function(r, t) {
    var o = window.require.on("error", t);
    window.require(e, function() {
      for (var a = [], d = 0; d < arguments.length; d++)
        a[d] = arguments[d];
      o.remove(), r(a);
    });
  });
}
function Xe(e, r) {
  if (r === void 0 && (r = {}), te())
    return he(e);
  var t = ze(), o = t && t.getAttribute("src");
  return !r.url && o && (r.url = o), Ce(r).then(function() {
    return he(e);
  });
}
const De = (e, r) => e.replace(/\(\?\<(.+?)\>[^)]*\)/g, (t, o) => r[o]);
Number.prototype.toRad = function() {
  return this * (Math.PI / 180);
};
Number.prototype.toDeg = function() {
  return this * (180 / Math.PI);
};
Number.prototype.to2DP = function() {
  return parseFloat(this.toFixed(2));
};
const be = function(e) {
  return Array.isArray(e) ? e : [
    e.longitude || (e.coords ? e.coords.longitude : e.geometry ? e.geometry.longitude : null),
    e.latitude || (e.coords ? e.coords.latitude : e.geometry ? e.geometry.latitude : null)
  ];
}, ve = function(e, r) {
  const [t, o] = be(e), [a, d] = be(r), p = a.toRad(), x = d.toRad(), v = t.toRad(), Z = o.toRad(), P = Math.sin(p - v) * Math.cos(x), H = Math.cos(Z) * Math.sin(x) - Math.sin(Z) * Math.cos(x) * Math.cos(p - v);
  return (Math.atan2(P, H).toDeg() + 360) % 360;
}, C = (e, r = {}, t = "") => {
  const o = document.createElement(e);
  for (let a in r)
    o.setAttribute(a, r[a]);
  return o.innerHTML = t, o;
}, Je = (e, r) => (document.getElementById(e) || document.getElementsByTagName("head")[0].prepend(C("STYLE", { type: "text/css" }, r)), !0), Ie = 600, Fe = 8, we = (e, r) => {
  const t = Number(e);
  return Number.isFinite(t) && t > 0 ? t : r;
}, F = (e, r) => {
  if (Array.isArray(e))
    return [0, 1, 2].map((t) => we(e[t], r));
  if (typeof e == "string")
    try {
      const t = JSON.parse(e);
      return Array.isArray(t) ? F(t, r) : F([t, t, t], r);
    } catch {
      return [r, r, r];
    }
  if (typeof e == "number") {
    const t = we(e, r);
    return [t, t, t];
  }
  return [r, r, r];
}, Qe = "data:image/png;base64,", Ke = function(e = {}) {
  Je("geocam-argis-map", `
      .geocam-auto-rotate-checkbox {
        display: none;
      }

      .geocam-auto-rotate-span {
        background-image: url("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' enable-background='new 0 0 24 24'  viewBox='0 0 24 24' fill='currentColor'><rect fill='none' height='24' width='24'/><path d='M7.94,5.12L6.49,3.66C8.07,2.61,9.96,2,12,2c5.52,0,10,4.48,10,10c0,2.04-0.61,3.93-1.66,5.51l-1.46-1.46 C19.59,14.86,20,13.48,20,12c0-4.41-3.59-8-8-8C10.52,4,9.14,4.41,7.94,5.12z M17.66,9.53l-1.41-1.41l-2.65,2.65l1.41,1.41 L17.66,9.53z M19.78,22.61l-2.27-2.27C15.93,21.39,14.04,22,12,22C6.48,22,2,17.52,2,12c0-2.04,0.61-3.93,1.66-5.51L1.39,4.22 l1.41-1.41l18.38,18.38L19.78,22.61z M16.06,18.88l-3.88-3.88l-1.59,1.59l-4.24-4.24l1.41-1.41l2.83,2.83l0.18-0.18L5.12,7.94 C4.41,9.14,4,10.52,4,12c0,4.41,3.59,8,8,8C13.48,20,14.86,19.59,16.06,18.88z'/></svg>")
      }

      .geocam-auto-rotate-checkbox:checked +.geocam-auto-rotate-span {
          background-image: url("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' enable-background='new 0 0 24 24' viewBox='0 0 24 24' fill='currentColor'><rect fill='none' height='24' width='24'/><path d='M18.6,19.5H21v2h-6v-6h2v2.73c1.83-1.47,3-3.71,3-6.23c0-4.07-3.06-7.44-7-7.93V2.05c5.05,0.5,9,4.76,9,9.95 C22,14.99,20.68,17.67,18.6,19.5z M4,12c0-2.52,1.17-4.77,3-6.23V8.5h2v-6H3v2h2.4C3.32,6.33,2,9.01,2,12c0,5.19,3.95,9.45,9,9.95 v-2.02C7.06,19.44,4,16.07,4,12z M16.24,8.11l-5.66,5.66l-2.83-2.83l-1.41,1.41l4.24,4.24l7.07-7.07L16.24,8.11z'/></svg>")
      }

    `);
  let t, o, a = [], d, p, x, v, Z, P, H, X, ne, re, D, Te, oe, Le, se = !0, ae;
  const { mapView: s, prevNextPlugin: Q, widgets: _e, expands: et, src: J } = e, S = document.createElement("div"), ie = function(n, i, c) {
    return {
      geometry: {
        type: "point",
        latitude: c,
        longitude: i
      },
      symbol: {
        type: "picture-marker",
        // autocasts as new PictureMarkerSymbol()
        url: Qe,
        width: "56",
        height: "101",
        angle: n
      }
    };
  };
  let k = ie(0, 0, 0);
  const ke = function(n) {
    return {
      attributes: {
        id: "lock"
      },
      geometry: {
        type: "point",
        latitude: n.latitude,
        longitude: n.longitude
      },
      symbol: {
        type: "simple-marker",
        // autocasts as new SimpleMarkerSymbol()
        path: "M12 4C7.31 4 3.07 5.9 0 8.98L12 21l5-5.01V8h5.92C19.97 5.51 16.16 4 12 4zm7 14h2v2h-2z M19 10h2v6h-2z",
        color: [255, 0, 0, 0.8],
        //"rgba(0,0,0,0)", for some reason if color isn't changed then outline colour doesn't change either.
        size: 24,
        // pixels
        xoffset: 0,
        yoffset: 12,
        outline: {
          // autocasts as new SimpleLineSymbol()
          color: [255, 255, 255, 1],
          width: 0
          // points
        }
      }
    };
  };
  let K = null;
  const Y = function(n, i, c) {
    d && (d.removeAll(), t.visible() ? (S.classList.remove("esri-disabled"), n !== null && (H() ? (s.rotation = n * -1, n = 0, (c || c === 0) && s.goTo({
      center: [i, c]
    })) : s.rotation = 0, k = ie(
      n || 0,
      i || k.geometry.longitude,
      c || k.geometry.latitude
    ), d.add(k), (c || c === 0) && ae([i, c]))) : S.classList.add("esri-disabled"));
  }, ce = function(n, i, c = {}) {
    const l = new RegExp(i, "i");
    let h = l.test(n.name) || l.test(n.alias);
    return h && c.description && (h = !!n.description), h;
  }, le = function(n) {
    if (n) {
      var i = document.createElement("textarea");
      return i.innerHTML = n, i.value;
    } else
      return "";
  }, Ee = (n, i) => {
    const c = n.base;
    if (n.filenames)
      return JSON.parse(i[n.filenames]).map((l) => Array.isArray(l) ? l.map((h) => /^https?:\/\//i.test(h) ? l : `${c}${h}`) : /^https?:\/\//i.test(l) ? l : `${c}${l}`);
    {
      const l = i[n.capture].split(".")[0], h = l.split("/").pop(), E = JSON.parse(i[n.lengths]), w = JSON.parse(i[n.offsets]);
      return E.map((A, m) => {
        const j = encodeURIComponent(
          `https://s3.us-west-004.backblazeb2.com/gc-raw-surveys-archive/${l}_${m}.tar`
        );
        return `${c}${h}/${m}/${i[n.shot]}.jpg?offset=${w[m]}&length=${A}&container=${j}`;
      });
    }
  }, je = (n) => {
    if (Array.isArray(n))
      return n.filter((i) => typeof i == "string");
    if (typeof n == "string")
      try {
        const i = JSON.parse(n);
        if (Array.isArray(i))
          return i.filter((c) => typeof c == "string");
      } catch {
        return [];
      }
    return [];
  }, Me = function(n) {
    return a.findIndex((i) => i.layer == n.layer);
  }, Ge = function(n) {
    if (t.label) {
      const { capture: i, utc_time: c, shot: l } = n, h = new Date(c);
      t.label(`${h.toLocaleString()}`);
    }
  };
  let ue;
  const $ = function(n, i) {
    const c = D();
    console.log("shotclick with viewlock", c);
    const l = a[i], h = n.attributes[l.shot];
    ue = h, t.shot(h), Q && (Q.prev(n.attributes.prev), Q.next(n.attributes.next));
    const E = [0, 1, 2].map(
      (M) => De(l.calibrationBase, {
        camera: M,
        rig_id: n.attributes[l.rigId],
        calibration: n.attributes[l.calibration]
      })
    ), w = n.attributes[l.yaw], W = n.attributes[l.rotation], A = X() && l.brightness ? n.attributes[l.brightness] : null;
    ne = A;
    const m = F(
      n.attributes.exposure_us,
      Ie
    ), j = F(
      n.attributes.gain_boost,
      Fe
    ), z = {
      id: h,
      latitude: n.geometry && typeof n.geometry.latitude == "number" ? n.geometry.latitude : null,
      longitude: n.geometry && typeof n.geometry.longitude == "number" ? n.geometry.longitude : null,
      utc_time: n.attributes.utc_time || null,
      capture: n.attributes[l.capture] || null,
      exposure_us: m,
      gain_boost: j,
      tags: je(n.attributes.image_tags)
    }, y = Ee(l, n.attributes);
    if (c) {
      const M = ve(n.geometry, c);
      t.facing(M);
    }
    t.show(y, w, E, W, A, z), Y(
      t.facing(),
      n.geometry.longitude,
      n.geometry.latitude
    ), Ge(n.attributes);
  };
  let de, q, _;
  const me = function(n, i, c, l) {
    clearTimeout(de), de = setTimeout(() => {
      const E = Math.ceil(n / 500), w = s.extent, W = `${w.xmin},${w.ymin},${w.xmax},${w.ymax},${w.spatialReference.wkid}`, A = `mod(id,${E}) = 0 AND extent = ${W}`;
      a.forEach((m) => {
        m.layer.definitionExpression !== A && (m.layer.definitionExpression = A, console.log(
          "definition expression changed for",
          m.layer,
          A
        ));
      }), re(s.zoom), Y(t.facing());
    }, 500);
  }, Oe = function(n, i, c, l) {
    oe([s.center.longitude, s.center.latitude]);
  };
  this.init = async function(n) {
    t = n, ae = t.store("marker"), re = t.store("zoom"), oe = t.store("center"), D = t.store("viewlock"), H = t.store("autorotate"), P = C("DIV", { class: "geocam-auto-rotate" });
    const i = C("LABEL", { class: "geocam-auto-rotate-label" }), c = C("INPUT", {
      type: "checkbox",
      class: "geocam-auto-rotate-checkbox"
    }), l = C(
      "SPAN",
      { class: "geocam-auto-rotate-span geocam-viewer-control-button" },
      " Autorotate"
    );
    c.checked = H(), c.addEventListener("change", () => {
      H(c.checked);
    }), i.appendChild(c), i.appendChild(l), P.appendChild(i), t.addControl(P, "left-top"), x = H((m) => {
      P.setAttribute(
        "title",
        m ? "turn auto-rotate off" : "turn auto-rotate on"
      ), Y(
        t.facing(),
        k.geometry.longitude,
        k.geometry.latitude
      );
    }), X = t.store("autobrightness"), v = X((m) => {
      t.reload(m ? ne : "[1,1,1]");
    }), Z = t.visible((m) => Y(t.facing()));
    const [h, E, w, W, A] = await Xe([
      "esri/layers/GraphicsLayer",
      "esri/core/watchUtils",
      "esri/layers/FeatureLayer",
      "esri/widgets/Expand",
      "esri/renderers/support/jsonUtils"
    ]);
    if (s.when(async () => {
      s.on("clickable", (u) => {
        se = u;
      }), s.on("key-down", (u) => {
        if (u && u.target && u.target.closest("input,calcite-input"))
          return;
        const f = [
          "ArrowUp",
          "ArrowDown",
          "ArrowRight",
          "ArrowLeft",
          "a",
          "d",
          "w",
          "s"
          // w and s don't seem to be used for map actions but just in case that changes in the future.
        ], g = u && u.key;
        t.visible() && f.indexOf(g) !== -1 && u.stopPropagation();
      }), s.on("immediate-click", (u) => {
        if (!se) return;
        const f = {
          x: u.x,
          y: u.y
        };
        if (console.log("immediate-click", u, f), o) {
          console.log("space wqas down");
          const g = s.toMap(f);
          if (D(g), K && s.graphics.removeAll(), K = ke(g), s.graphics.add(K), t.visible()) {
            const T = ve(k.geometry, g);
            t.facing(T);
          }
        } else
          s.hitTest(f).then((g) => {
            if (g.results && g.results.length > 0)
              for (var T = 0; T < g.results.length; T++) {
                const L = g.results[T].graphic, N = Me(L);
                if (N >= 0) {
                  Object.entries(L.attributes).length < 2 ? L.layer.queryFeatures({
                    objectIds: [L.attributes.id],
                    returnGeometry: !0,
                    outFields: "*",
                    where: L.layer.definitionExpression
                  }).then((G) => {
                    G.features.length > 0 && $(G.features[0], N);
                  }) : $(L, N);
                  break;
                }
              }
          });
      });
      const m = document.createElement("div");
      m.className = "esri-widget--button", m.title = "Copy short URL to clipboad", m.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" height="24" width="24"><path d="M23 15H11.707l2.646 2.646-.707.707L9.793 14.5l3.854-3.854.707.707L11.707 14H23zm-13-5H6v1h4zm-4 5h2v-1H6zM3 4h3V3h3a2 2 0 0 1 4 0h3v1h3v9h-1V5h-2v2H6V5H4v16h14v-5h1v6H3zm4 2h8V4h-3V2.615A.615.615 0 0 0 11.386 2h-.771a.615.615 0 0 0-.615.615V4H7zM6 19h4v-1H6z"></path></svg>
    <span class="esri-icon-font-fallback-text">Copy short URL to clipboad</span>`, m.addEventListener("click", async () => {
        const u = `${document.location.origin}/🔗`;
        try {
          const g = await (await fetch(u, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              link: { url: document.location.toString() }
            })
          })).json();
          await navigator.clipboard.writeText(g.link), alert(`Url copied to clipboard: ${g.link}`);
        } catch (f) {
          alert(
            `Sorry, the short URL could not be copied to the clipboard: ${f}`
          );
        }
      }), s.ui.add(m, "top-right"), S.className = "esri-widget--button", S.title = "Recenter map on selected shot", S.innerHTML = '<span aria-hidden="true" class="esri-icon-zoom-to-object"></span><span class="esri-icon-font-fallback-text">Expand</span>', S.addEventListener("click", () => {
        s.goTo({
          center: [k.geometry.longitude, k.geometry.latitude]
        });
      }), s.ui.add(S, "top-right"), p = t.facing((u) => {
        Y(u);
      });
      const j = new URLSearchParams(window.location.hash.substr(1)), z = JSON.parse(j.get("center") || null);
      z && (console.log("got center from hash params", z), s.center = z, _ = !0);
      const y = JSON.parse(j.get("zoom") || null);
      y && (console.log("got zoom from hash params", y), s.zoom = y);
      const M = j.get("marker");
      if (M) {
        const u = JSON.parse(M);
        if (u) {
          const [f, g] = u;
          Y(t.facing(), f, g);
        }
      }
      E.watch(s, "scale", me), me(s.scale), E.watch(s, "center", Oe), t.shot((u) => {
        const f = parseInt(
          typeof u == "object" && u !== null ? u.id : u
        );
        if (f && f !== ue) {
          const g = function() {
            a.forEach((T, L) => {
              const N = T.layer;
              t.resetProgress(), console.log("Querying layer for shot", N, f), N.queryFeatures({
                objectIds: [f],
                returnGeometry: !0,
                outFields: "*",
                where: N.definitionExpression
              }).then((G) => {
                if (console.log("Got results for layer", N, G), G.features.length > 0) {
                  const b = G.features[0];
                  $(b, L);
                }
              });
            });
          };
          if (console.log("Got shot", u, "layers", a.length), a.length === 0) {
            q = g;
            return;
          }
          g();
        } else
          u || t.hide();
      });
    }), J) {
      const m = `${J}/2`, j = new w({
        url: m,
        visible: !1,
        outFields: ["*"],
        editingEnabled: !0
      });
      s.map.add(j);
      const z = `${J}/0`;
      console.log("shots url is", z);
      const y = new w({
        url: z,
        definitionExpression: "mod(id,100) = 0"
        // start with agressive simplifaction - view should get scale change early on to override this
      });
      s.map.add(y), y.when((f) => {
        const g = f.fields, T = g.find((b) => ce(b, "filenames")), L = g.find((b) => ce(b, "calibration"));
        a.push({
          layer: y,
          shot: "id",
          filenames: "filenames",
          yaw: "yaw",
          rotation: "rotation_matrix",
          datetime: "utc_time",
          brightness: null,
          base: le(T && T.description),
          calibration: "calibration",
          rigId: null,
          calibrationBase: le(L.description),
          capture: "capture"
        }), _ ? (console.log("center was set"), _ = !1) : (console.log("center not set using layer extent", f.fullExtent), s.extent = f.fullExtent), q && (console.log("setting deferred shot", q), q(), q = null);
        const N = (b) => {
          if (!Array.isArray(b) || b.length === 0) return;
          const U = document.createElement("div");
          U.style.padding = "10px", U.style.background = "white", U.style.minWidth = "150px";
          const B = document.createElement("div");
          B.style.marginBottom = "6px", B.style.fontWeight = "bold", B.style.fontSize = "13px", B.textContent = "Symbology", U.appendChild(B), b.forEach((R) => {
            const O = document.createElement("input");
            O.type = "radio", O.name = "geocam-drawing-info", O.value = R.name, O.id = `geocam-di-${R.name}`, O.style.marginRight = "6px", R.name === "Original" && (O.checked = !0, y.renderer = A.fromJSON(R.renderer)), O.addEventListener("change", () => {
              y.renderer = A.fromJSON(R.renderer);
            });
            const V = document.createElement("label");
            V.htmlFor = O.id, V.textContent = R.name, V.style.fontSize = "13px", V.style.cursor = "pointer";
            const I = document.createElement("div");
            I.style.marginBottom = "4px", I.appendChild(O), I.appendChild(V), U.appendChild(I);
          });
          const Ue = new W({
            view: s,
            content: U,
            expandIconClass: "esri-icon-hollow-eye",
            expandTooltip: "Symbology options"
          });
          s.ui.add(Ue, "top-right");
        }, G = f.sourceJSON && f.sourceJSON.drawingInfo && f.sourceJSON.drawingInfo.drawingInfoOptions;
        G ? N(G) : fetch(`${z}?f=json`).then((b) => b.json()).then((b) => {
          const U = b.drawingInfo && b.drawingInfo.drawingInfoOptions;
          N(U);
        }).catch((b) => {
          console.warn("Failed to fetch drawingInfoOptions", b);
        });
      });
      const M = `${J}/1`;
      console.log("points features url is", M);
      const u = new w({
        url: M,
        popupEnabled: !0,
        popupTemplate: {
          title: "{reference}",
          content: [
            {
              type: "fields",
              fieldInfos: [
                {
                  fieldName: "embed",
                  label: "content"
                }
              ]
            }
          ]
        }
      });
      s.map.add(u), d = new h({
        title: "GeoCam Field of View",
        geometryType: "point",
        spatialReference: {
          wkid: 4326
        }
      }), s.map.layers.add(d), s.when(() => {
        console.log("mapview when"), s.map.reorder(d, 1e3), s.map.reorder(y, 1e3), s.map.reorder(u, 1e3);
      });
    }
  };
  var fe = function(n) {
    switch (n.key, n.key) {
      case "Escape": {
        D(null), s.graphics.removeAll();
        break;
      }
      case " ":
        o = !0;
    }
  }, ge = function(n) {
    switch (n.key, n.key) {
      case " ":
        o = !1;
    }
  };
  document.addEventListener("keydown", fe), document.addEventListener("keyup", ge), this.destroy = function() {
    document.removeEventListener("keydown", fe), document.removeEventListener("keyup", ge), p(), x(), v(), Te(), Le(), Z(), s.map.removeLayer(d), t.wrapper.removeChild(P);
  };
};
class $e extends HTMLElement {
  constructor() {
    super(), this.plugin = null, console.log("GeocamViewerArcgisMap init");
  }
  connectedCallback() {
    const r = this.closest("geocam-viewer");
    if (!r) {
      console.error("GeocamViewerArcgisMap must be a child of GeocamViewer");
      return;
    }
    const t = (o) => {
      const a = r.viewer;
      a && typeof a.plugin == "function" ? o(a) : setTimeout(() => t(o), 50);
    };
    this.link = (o) => {
      console.log("linking to ", o);
      const a = this.getAttribute("src");
      a || console.warn("No src attribute on geocam-viewer-arcgis-map"), t((d) => {
        if (this.plugin)
          return;
        this.viewer = d, this.mapView = o;
        const p = r.querySelector("geocam-viewer-prev-next-control"), x = p && p.plugin;
        this.plugin = new Ke({ mapView: o, prevNextPlugin: x, src: a }), this.viewer.plugin(this.plugin);
        const v = r.querySelector("geocam-viewer-screen-shot");
        v && v.plugin && v.plugin.arcgisView(o);
      });
    }, console.log("GeocamViewerArcgisMap connected");
  }
  disconnectedCallback() {
    this.plugin = null, this.viewer = null, this.mapView = null, console.log("GeocamViewerArcgisMap disconnected");
  }
}
window.customElements.define("geocam-viewer-arcgis-map", $e);
export {
  $e as GeocamViewerArcgisMap
};
